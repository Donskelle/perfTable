  ...
  class PerfTester extends HTMLElement {
    ...
    set chart(value) {
      ...
      if (value && !this._chartLoaded) {
        ...
        script.onerror = function(e) {
          console.error(`couldn't load chart lib: ${e}`);
        };
    ...
    _drawCharts() {
      if (this._shouldDrawChart) {
        for (let i = 0; i < this.tests.length; i++) {
          let data = [[this.tests[i]]].concat(
            this._times[i].map(function(t) {
              return [t];
            })
          );
          ...
          let chart = new window.google.visualization.Histogram(
            this.shadowRoot.querySelector('#chart' + i)
          );
    ...
    _calcScores(times, baseline) {
      ...
      for (let kind in this._strategies) {
        ...
        let compare = baseline ? (time / baseline[kind].time).toFixed(2) : 1;
    ...
    _stats(a, kind) {
      let r = { mean: 0, letiance: 0, deviation: 0 },
        t = a.length;
      let m, s, l;
      for (s = 0, l = t; l--; s += a[l]);
